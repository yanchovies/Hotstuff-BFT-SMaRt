 ...
 */
package hotstuff.tom.core;

import hotstuff.communication.ServerCommunicationSystem;
import hotstuff.consensus.Consensus;
import hotstuff.consensus.Decision;
import hotstuff.consensus.Epoch;
import hotstuff.consensus.TimestampValuePair;
import hotstuff.consensus.messages.ConsensusMessage;
import hotstuff.consensus.messages.KeyShareMessage;
import hotstuff.consensus.messages.SigShareMessage;
import hotstuff.consensus.roles.Backup;
import hotstuff.reconfiguration.ServerViewController;
import hotstuff.tom.util.thresholdsig.Thresig;
import hotstuff.statemanagement.StateManager;
import hotstuff.tom.core.messages.TOMMessage;
import hotstuff.tom.leaderchange.*;
import hotstuff.tom.util.BatchBuilder;
import hotstuff.tom.util.BatchReader;
import hotstuff.tom.util.TOMUtil;
...
public class Synchronizer {
	...
	// still not in the leader change phase?
        if (lcManager.getNextReg() == lcManager.getLastReg()) {
            ...
            try { // serialize content to send in STOP message
                ...
                // send STOP-message                
                logger.debug("Sending STOP message to install regency " + regency + " with " + (messages != null ? messages.size() : 0) + " request(s) to relay");
    ...
    private void processOutOfContextSTOPs(int regency) {
        ...
        if (stops.size() > 0) {
            logger.debug("Processing " + stops.size() + " out of context STOPs for regency " + regency);
        } else {
    ...
    private void processSTOPDATA(LCMessage msg, int regency) {
        ...
        try { // deserialize the content of the message
            ...
            if (ois.readBoolean()) { // content of the last decided cid
                ...
                n = null;//(BigInteger) ois.readObject();
                e = null;//(BigInteger) ois.readObject();
    ...
    private void startSynchronization(int nextReg) {
        ...
        if (condition && lcManager.getNextReg() == lcManager.getLastReg()) {
            ...
            try { // serialize conent to send in the STOP message
                ...
                logger.debug("Could not deserialize STOP message"+ex);
            }
        }
        ...
        if (condition && lcManager.getNextReg() > lcManager.getLastReg()) {
            ...
            if (leader != this.controller.getStaticConf().getProcessId()) {
                ...
                try { // serialize content of the STOPDATA message
                    ...
                    logger.debug("Sending STOPDATA of regency " + regency);
		...
		//TODO: Turn on timeout again?
                ...
                if (sync.size() > 0) {
                    logger.debug("Processing out of context SYNC for regency " + regency);
                } else {
                    logger.debug("No out of context SYNC for regency " + regency);
                }
    ...
    public void deliverTimeoutRequest(LCMessage msg) {
        ...
        switch (msg.getType()) {
            case TOMUtil.STOP: { // message STOP
                ...
                logger.debug("Last regency: " + lcManager.getLastReg() + ", next regency: " + lcManager.getNextReg());
    ...
    private void catch_up(int regency) {
		...
		//TODO: Missing: serialization of the proof?
                ...
                logger.debug("Sending SYNC message for regency " + regency);
    ...
    private void finalise(int regency, CertifiedDecision lastHighestCID,
        ...
        if (tom.getLastExec() + 1 < lastHighestCID.getCID()) { // is this a delayed replica?
            ...
            logger.debug("NEEDING TO USE STATE TRANSFER!! (" + lastHighestCID.getCID() + ")");
...
/******************************************************************************************************************/
            ...
            logger.debug("(Synchronizer.finalise) I'm still at the CID before the most recent one!!! (" + lastHighestCID.getCID() + ")");
...
//            }
        ...
        if (e != null){//e != null) {
            ...
            logger.debug("Installed proof of last decided consensus " + lastHighestCID.getCID());
            ...
            if (tom.getLastExec() + 1 == lastHighestCID.getCID()) {
                ...
                logger.debug("I'm still at the CID before the most recent one!!! (" + lastHighestCID.getCID() + ")");
                cons.decided(e, true);
            }
            else {
            ...
            logger.debug("I did not install any proof of last decided consensus " + lastHighestCID.getCID());
        }
        cons = null;
        e = null;
        ...
        if (tmpval != null) { // did I manage to get some value?
            ...
            if (regency > ets) {
                ...
                //logger.debug("(Synchronizer.finalise) Updating consensus' ETS after SYNC (from " + ets + " to " + currentETS +")");
...
/*******************************************7.13************************************************************/
            ...
            if (iAmLeader) {
                ...
                logger.debug("Sending PREPARE message for CID " + currentCID + ", timestamp " + e.getTimestamp() + ", value " + Arrays.toString(e.propValueHash));
