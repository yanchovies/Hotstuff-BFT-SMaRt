...
import hotstuff.consensus.messages.MessageFactory;
import hotstuff.consensus.roles.Backup;
import hotstuff.consensus.roles.Primary;
import hotstuff.tom.core.ExecutionManager;
import hotstuff.reconfiguration.ReconfigureReply;
import hotstuff.reconfiguration.ServerViewController;
import hotstuff.reconfiguration.VMMessage;
import hotstuff.tom.core.ReplyManager;
import hotstuff.tom.core.TOMLayer;
import hotstuff.tom.core.messages.TOMMessage;
import hotstuff.tom.core.messages.TOMMessageType;
import hotstuff.tom.leaderchange.CertifiedDecision;
import hotstuff.tom.server.BatchExecutable;
import hotstuff.tom.server.Executable;
import hotstuff.tom.server.Recoverable;
import hotstuff.tom.server.Replier;
import hotstuff.tom.server.RequestVerifier;
import hotstuff.tom.server.SingleExecutable;

import hotstuff.tom.server.defaultservices.DefaultReplier;
import hotstuff.tom.util.KeyLoader;
import hotstuff.tom.util.ShutdownHookThread;
                    ...
                    if (null == request.getReqType()) {
                        ...
                        case ORDERED_REQUEST:
                            ...
                            MessageContext msgCtx = new MessageContext(request.getSender(), request.getViewID(),
                                    ...
                                    consId[consensusCount], cDecs[consensusCount].getSigsMsgs(), cDecs[consensusCount].getN(), cDecs[consensusCount].getE(), firstRequest, false);
                ...
                if (requestsFromConsensus.length > 0) {
                    ...
                    for (TOMMessage m : requestsFromConsensus) {
                        ...
                        msgCtx[line] = new MessageContext(m.getSender(), m.getViewID(),
                                ...
                                consId[consensusCount], cDecs[consensusCount].getSigsMsgs(), cDecs[consensusCount].getN(), cDecs[consensusCount].getE(), firstRequest, true);
